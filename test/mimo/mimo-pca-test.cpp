...
TEST_CASE("MiMo-PCA")
{
    ...
    GIVEN("A M*N input matrix m1:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u1), pca.U)); // check u
            ...
            pca.decomposition.VT = xTranspose(std::get<0>(vlm1), n, rank); //because our feature space is slightly different I reassign VT from matlab
    ...
    GIVEN("A M*M square input matrix m2:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u2), pca.U)); // check u
    ...
    GIVEN("A M*M diagonal input matrix m3:")
    {
        ...
        std::get<0>(m3).clear();
        std::get<0>(m3).resize(std::get<1>(m3)*std::get<2>(m3));
        for(int i = 0, j = 0; i < std::get<1>(m3)*std::get<2>(m3);++j, i+=(std::get<1>(m3)+1))
            std::get<0>(m3)[i] = diagvals[j];
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u3), pca.U)); // check u
    ...
    GIVEN("A M>N rectangular input matrix m4:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u4), pca.U)); // check u
    ...
    GIVEN("A M<N rectangular input matrix m5:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u5), pca.U)); // check u
    ...
    GIVEN("A M*M diagonal input matrix m6:")
    {
        ...
        std::get<0>(m6).clear();
        std::get<0>(m6).resize(std::get<1>(m6)*std::get<2>(m6));
        for(int i = 0, j = 0; i < std::get<1>(m6)*std::get<2>(m6);++j, i+=(std::get<1>(m6)+1))
            std::get<0>(m6)[i] = diagvals[j];
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(std::get<0>(u6), pca.U)); // check u
    ...
    //In the next four tests we feed the program matrices with either the same columns or the same rows.
    //Only the first dimension should be relevant in the resulting matrix of singular values. Thus we
    //check the whole matrix of S and a forward transformation with only the the first column of V
    GIVEN("A matrix with same cols, square m7:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(parent.values, std::get<0>(fw7).data(), rank));
        }
    }
    GIVEN("A matrix with same cols, square m8:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(parent.values, std::get<0>(fw8).data(), rank));
        }
    }
    GIVEN("A matrix with same cols, rect m9:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(parent.values, std::get<0>(fw9).data(), rank));
        }
    }
    GIVEN("A matrix with same cols, rect m10:")
    {
        ...
        THEN("Decomposition and transformation should result in")
        {
            ...
            CHECK(vecIsAbsAprox(parent.values, std::get<0>(fw10).data(), rank));
        }
    }
    //Special case zero matrix: any U and VT are correct, S should result in a zero matrix.
